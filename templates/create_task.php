    ...
    <main class="ml-64 flex-1 p-6">
        ...
        <form method="post" action="/create_task.php" class="bg-card-bg p-6 rounded-xl shadow-card-lg space-y-6 max-w-xl">
            <div>
                ...
                <input name="title" required class="w-full px-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary" value="<?= htmlspecialchars($_POST['title'] ?? '') ?>">
            </div>
            <div>
                <label class="block text-sm font-medium text-text-secondary mb-1">Beschreibung</label>
                <textarea name="description" rows="3" class="w-full px-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-text-secondary mb-1">Empfänger *</label>
                <select name="assigned_to" required class="w-full px-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary">
                    <option value="" disabled selected>Bitte auswählen…</option>
                    <?php foreach ($allUsers as $u): ?>
                        <option value="<?= $u['id'] ?>" <?= ((isset($_POST['assigned_to']) && $_POST['assigned_to'] == $u['id']) ? 'selected' : '') ?>><?= htmlspecialchars($u['username']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-text-secondary mb-1">Fällig am</label>
                <input type="date" name="due_date" class="w-full px-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary" value="<?= htmlspecialchars($_POST['due_date'] ?? '') ?>">
            </div>
            <button type="submit" class="px-6 py-2 bg-primary text-white rounded-lg shadow hover:bg-primary-dark transition">
                Aufgabe erstellen
            </button>
        </form>
    </main>
