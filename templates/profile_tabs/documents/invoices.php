...
// Rechnungen mit modernem liquid glass design
if (!isset($docs) || !isset($_SESSION['user_id'])) {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE d.user_id = ? AND (dc.name = 'Rechnungen' OR dc.name LIKE '%rechnung%') AND d.is_deleted = 0
...
$invoiceDocs = array_filter($docs, function($doc) {
    return stripos($doc['category_name'] ?? '', 'rechnung') !== false || 
           ($doc['category_name'] ?? '') === 'Rechnungen';
});
?>
...
<div class="space-y-6">
  ...
  <div class="glass-card p-8">
    <div class="flex items-center justify-between mb-4">
      <div>
        <h2 class="text-3xl font-bold text-white mb-2 bg-gradient-to-r from-green-400 to-blue-400 bg-clip-text text-transparent">
          Meine Rechnungen
        </h2>
        <p class="text-white/70">Verwalten Sie Ihre Rechnungen und Belege</p>
      </div>
      <div class="text-right">
        <div class="text-sm text-white/60 mb-1">Rechnungen</div>
        <div class="text-2xl font-bold text-white"><?= count($invoiceDocs) ?></div>
      </div>
    </div>
    ...
    <?php if (empty($invoiceDocs)): ?>
      <div class="glass-card p-12 text-center">
        <div class="flex flex-col items-center gap-6">
          <div class="w-24 h-24 bg-gradient-to-br from-green-500/20 to-blue-500/20 rounded-2xl flex items-center justify-center">
            <svg class="w-12 h-12 text-white/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
            </svg>
          </div>
          <div>
            <h3 class="text-xl font-semibold text-white mb-2">Keine Rechnungen gefunden</h3>
            <p class="text-white/60">Laden Sie Ihre ersten Rechnungen hoch</p>
          </div>
        </div>
      </div>
    <?php else: ?>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <?php foreach($invoiceDocs as $doc): ?>
          <div class="document-card group">
            <div class="document-card-inner">
              ...
              <div class="p-6 pb-4">
                ...
                <p class="document-subtitle">Rechnung</p>
              </div>
        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-white/10">
              <th class="text-left py-4 px-6 font-medium text-white/80">Rechnung</th>
              ...
              <tr>
                <td colspan="4" class="py-12 text-center">
                  <div class="flex flex-col items-center gap-4">
                    <div class="w-16 h-16 bg-gradient-to-br from-green-500/20 to-blue-500/20 rounded-2xl flex items-center justify-center">
                      <svg class="w-8 h-8 text-white/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                      </svg>
                    </div>
                    <div>
                      <h3 class="text-lg font-semibold text-white mb-1">Keine Rechnungen gefunden</h3>
                      <p class="text-white/60 text-sm">Laden Sie Ihre ersten Rechnungen hoch</p>
                    </div>
                  </div>
                </td>
              </tr>
